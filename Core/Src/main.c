...
volatile AppState state = { .mode = CYCLING, .timeOfDay = DAY, .dayFrame = {
		.address = 4, .command = 0x58 }, .nightFrame = { .address = 4,
		.command = 0x19 }, .twilightFrame = { .address = 4, .command = 0x5C } };
NEC nec;
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, LED_LEARN_Pin|LED_DAY_Pin|LED_NIGHT_Pin|LED_TWILIGHT_Pin, GPIO_PIN_RESET);
  ...
  GPIO_InitStruct.Pin = LED_LEARN_Pin|LED_DAY_Pin|LED_NIGHT_Pin|LED_TWILIGHT_Pin;
